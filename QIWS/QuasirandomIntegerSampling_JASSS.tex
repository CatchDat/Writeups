...
We show that by using without-replacement sampling we can guarantee that the randomly sampled population will exactly match the marginal data. Secondly we compare the statistical properties of populations generated with IPF, and sampled using pseudo- and quasirandom numbers  showing that the latter sampling technique provides far better degeneracy, closer to the maximal entropy solution that IPF converges to.
...
%Although situations do arise where the distributions diverge in such a way that it is not possible to sample from one and remove from the other, the population can often be adjusted to correct for this.  
...
funding for this work. We are very grateful to the anonymous reviewers who have provided helpful comments and guidance during the development of this manuscript.
