...
``toy'' examples we do not attribute any significance to this as IPF is known not to perform well in such cases (ref?).

\subsubsection{Randomness in Context}\label{randomness-in-context}

To put pseudorandom versus quasirandom sampling into context, firstly consider a
...
populations of each state (from their expected value), we can thus estimate a reasonable approximation for \(\chi^2\) as
\begin{equation}
\chi^2 \approx \frac{N^2}{P}X^2
